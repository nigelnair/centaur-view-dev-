...
import { UserData } from '../_model/userData.model';
...
// }
  ...
  constructor(
    ...
    updateAcc(c_id){
        ...
        let data = [{'username':user.username,'password': user.password,'cell': user.cell,'phone': user.phone,'email': user.email }
        ];
        this._loginService.sendAccountData({ data })
        .subscribe(
      ...
      validateEmail(value){
        ...
        if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(this.c_email_alt)) {
          this.save=1;
        }else{
          this.save=0;
